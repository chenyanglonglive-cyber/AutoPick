...
export type Preflight = { ready: boolean; total_slots: number; selected_slots: number; missing_slots: number; active_overrides: number; pending_aliases: number; feedback_records: number; message: string; checklist_type_id: string; factory_name: string }
...
export const api = {
  ...
  match: (id: string) => request<any>(`/api/projects/${id}/match`, { method: 'POST' }),
  job: (id: string, jobId: string) => request<any>(`/api/projects/${id}/jobs/${jobId}`),
  checklist: (id: string) => request<Slot[]>(`/api/projects/${id}/checklist`),
  ...
  search: (id: string, query: string) => request<Candidate[]>(`/api/projects/${id}/search`, { method: 'POST', body: JSON.stringify({ query, top_k: 30 }) }),
  ...
  confirmAllTop: (id: string) => request<any>(`/api/projects/${id}/slots/confirm-top`, { method: 'POST' }),
  preflight: (id: string) => request<Preflight>(`/api/projects/${id}/export/preflight`),
  exportExcel: (id: string) => request<any>(`/api/projects/${id}/export?allow_partial=true`, { method: 'POST' }),

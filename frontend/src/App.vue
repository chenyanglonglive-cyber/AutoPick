...
const galleryProcessing = computed(() => job.value?.kind === 'gallery_processing' && ['queued', 'running'].includes(job.value.status))
...
async function followGalleryProcessing(projectId: string, jobId: string) { try { while (job.value && ['queued', 'running'].includes(job.value.status)) { await new Promise(resolve => setTimeout(resolve, 800)); job.value = await api.job(projectId, jobId) }; if (!job.value || job.value.status !== 'completed') throw new Error(job.value?.message || '图库处理未完成，请重试。'); if (activeProject.value?.id === projectId) { await openProject(activeProject.value); if (activeView.value === 'gallery') await openGallery() }; message.value = job.value.message } catch (e: any) { error.value = e.message } }
async function processGallery() { if (!activeProject.value) return; try { const projectId = activeProject.value.id; job.value = await api.processGallery(projectId); message.value = '已在后台开始处理新增图片；你可以继续浏览和复核。'; void followGalleryProcessing(projectId, job.value.id) } catch (e: any) { error.value = e.message } }
  ...
  <main class="app-root">
    ...
    <section v-if="activeProject" :class="['workspace', { 'gallery-workspace': activeView === 'gallery' }]">
      ...
      <section v-else class="content-panel">
        <div class="gallery-heading"><div><p class="eyebrow">当前工厂专属图库</p><h1>图库浏览</h1><p class="muted">{{ galleryPhotos.length }} 张照片 · {{ galleryPhotos.filter(photo => photo.embedding_status === 'indexed').length }} 已向量化</p></div><div class="gallery-heading-actions"><div class="gallery-processing" aria-label="图库处理"><p>图库处理</p><div><button class="gallery-process-button" :disabled="loading || galleryProcessing" @click="processGallery">处理新增图片（向量 + OCR）</button></div></div><button class="secondary" :disabled="loading" @click="updateGallery">更新图库</button></div></div>
